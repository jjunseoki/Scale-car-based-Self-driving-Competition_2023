...
class Controller():
    ...
    def __init__(self):
        ...
        self.speed_rabacon = 0.3 # rabacon speed
        ...
        self.warning_speed = 0.5 # warning dynamic
        ...
        self.rabacon_angle = 0.0
        self.able_to_dynamic = 0
        ...
        # rospy.Subscriber("del", Float64, self.capstone_callback)
    
    # def capstone_callback(self, _data) :
    #     self.rabacon_angle = _data.data
    #     print(self.rabacon_angle , '@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
...
# ========== preprocessing via HSV ==========================
        ...
        # gym
        # lower_lane = np.array([175,150 , 175]) 
    ...
    def lane_detection(self, lane_image) :
        ...
        # equalized_color =cv2.equalizeHist(blur_img)
        # cv2.imshow('equail', equalized_color)
        ...
        # warped_img2 = self.warper.warp(close)
        # cv2.imshow("canny", warped_img2)

        warped_img = self.warper.warp(blur_img)
        # crop_coordinates = (100, 150, 200, 300)
        # cropped_img = warped_img[crop_coordinates[0]:crop_scoordinates[0] + crop_coordinates[2],
        #                  crop_coordinates[1]:crop_coordinates[1] + crop_coordinates[3]]
        # cv2.imshow("cropped", cropped_img)
        # cv2.imshow("warped_img", warped_img)
        self.slide_img, self.slide_x_location, self.current_lane_window = self.slidewindow.slidewindow(warped_img, self.lane_is_not_detect)
        if self.slide_x_location == None :
            self.slide_x_location = self.last_x_location
            self.lane_is_not_detect = False
        else :
            self.last_x_location = self.slide_x_location
            self.lane_is_not_detect = True
        cv2.imshow("slide_img", self.slide_img)
        # rospy.loginfo("CURRENT LANE WINDOW: {}".format(self.current_lane_window))
    
    def stop_traffic(self, lane_image) :
        # print(lane_image.shape)
        kernel_size = 5
        blur_img = cv2.GaussianBlur(lane_image,(kernel_size, kernel_size), 0)
        warped_img = self.warper.warp(blur_img)
        crop_coordinates = (0, 270, 20, 100)
        cropped_img = warped_img[crop_coordinates[0]:crop_coordinates[0] + crop_coordinates[2],
                         crop_coordinates[1]:crop_coordinates[1] + crop_coordinates[3]]
        
        # cv2.imshow("cropped", cropped_img)
        tt1 = rospy.get_time()
        binary_image = (cropped_img >= 180).astype(np.uint8)
        ...
        if white_ratio >= 0.5 :
            ...
            if(self.crosswalk_cnt > 3) :
                self.stop_flag = 1
        else :
            self.crosswalk_cnt = 0
        # print('whtie_ratio', white_ratio)
        return 

    def follow_lane(self): # 차선데이터를 받아서 차선을 따라서 주행하도록 하는 함수
        if self.able_to_dynamic == 1 :
            self.speed_lane = self.warning_speed
        else :
            self.speed_lane = self.default_speed # compettion_day
        # slow_down_sign 
        if self.slow_down_flag == 1:
            # self.stop_traffic(self.lane_image)
            #rospy.loginfo("sign data :{}".format(self.sign_data))
            if self.sign_data != 0:
                rospy.loginfo(" ===============   SLOW DETECTED, WAIT!!!! ============")
                self.error_lane = 300 - self.slide_x_location # error가 음수 --> 오른쪽 차선이랑 멈 / error가 양수 --> 오른쪽 차선이랑 가까움
                ...
                if self.slow_flag == 0:
                    ...
                    self.slow_flag = 1
                ##### 정지선 보기전에 천천히 가는 구간 #####
                elif self.slow_flag == 1 and self.stop_flag == 0: 
                    print('before stop lane')
                    self.error_lane = 300 - self.slide_x_location # error가 음수 --> 오른쪽 차선이랑 멈 / error가 양수 --> 오른쪽 차선이랑 가까움
                    publishing_data = AckermannDriveStamped()
                    publishing_data.header.stamp = rospy.Time.now() # 이 데이터를 보낼 때의 시점
                    publishing_data.header.frame_id = "base_link"
                    ...
                    if self.time_flag == 0 :
                        self.stop_t1 = rospy.get_time()
                        self.time_flag = 1
                        self.current_lane = "LEFT"
                    # stop_t1 = rospy.get_time()
                    # print(stop_t1, stop_t2)
                    rospy.loginfo("************* STOP *****************")
                    #rospy.loginfo("slow_down time{}, {}".format(self.slow_t1,t2))
                    # stop_t1 = rospy.get_time()
                    stop_t2 = rospy.get_time()
                    print(self.stop_t1, stop_t2)
                    if stop_t2-self.stop_t1 <= 5 and self.time_flag==1:
                        stop_t2 = rospy.get_time()
                        self.error_lane = 300 - self.slide_x_location # error가 음수 --> 오른쪽 차선이랑 멈 / error가 양수 --> 오른쪽 차선이랑 가까움
                        publishing_data = AckermannDriveStamped()
                        publishing_data.header.stamp = rospy.Time.now() # 이 데이터를 보낼 때의 시점
                        ...
                        self.stop_flag = 2
                        self.time_flag = 0
                # print('before stop lane')white_mask = cv2.inRange(cropped_img, lower_white, upper_white)
                    
                elif self.stop_flag == 2 and self.slow_flag == 1 :
                    if self.time_flag == 0 :
                            self.stop_t1 = rospy.get_time()
                            self.time_flag = 1
                    t2 = rospy.get_time()
                    ...
                    if t2-self.stop_t1 <= 15 and self.time_flag==1:
                        rospy.loginfo("************* SLOW DOWN 2*****************")
                        #rospy.loginfo("slow_down time{}, {}".format(self.slow_t1,t2))
                        self.error_lane = 300 - self.slide_x_location # error가 음수 --> 오른쪽 차선이랑 멈 / error가 양수 --> 오른쪽 차선이랑 가까움
                        publishing_data = AckermannDriveStamped()
                        publishing_data.header.stamp = rospy.Time.now() # 이 데이터를 보낼 때의 시점
            ...
            return
        # rabacon
        elif self.rabacon_mission == 1 and self.dynamic_flag == 0 and self.static_flag == 0:
            self.rabacon_drive()
        ...
        # obstacle
            ...
            elif abs(statistics.mean(self.y_list_sort[0:1]) - statistics.mean(self.y_list_sort[-2:-1])) >= 0.1 or abs(self.y_list_sort[10]) > 0.15 :
                self.dynamic_flag = 1
                self.static_flag = 0
                # self.y_list.clear
            ...
            # static obstacle
                ...
                if self.current_lane == "RIGHT":
                    ...
                    while t2-self.turn_left_t1 <= 1.2:
                        ...
                        t2 = rospy.get_time()
                    while t2- self.turn_left_t1 <= 1.55 :
                ...
                # if the car is driving depending on "left" window
                    ...
                    while t2-self.turn_right_t1 <= 1.2:
                        self.change_line_right()
                        t2 = rospy.get_time()
                    while t2-self.turn_right_t1 <= 1.45 :
                        self.change_line_left()
                        t2 = rospy.get_time()
        ...
        try :
            ...
            if _data.data == 100:
                ...
                if self.child_cnt >= 10 :
                    ...
                    self.child_cnt = 0
            # elif  _data.data == 101 and self.slow_down_flag == 1:
            #     self.child_cnt += 1
            #     if self.child_cnt >= 10 :
            #         self.sign_data = _data.data
            #         self.slow_down_flag = 2
            #         self.child_cnt = 0
            else :
                self.sign_data = 0
                # self.slow_down_flag = 0
    ...
    def rabacon_drive(self) :
        ...
        publishing_data.drive.steering_angle = self.rabacon_mission_flag * -1.7
        # print('##############', self.rabacon_mission_flag * -1.7)
        # publishing_data.drive.speed = max(self.speed_rabacon-0.2, self.speed_rabacon +0.1- abs(self.rabacon_mission_flag * -1.5)/2)
        # publishing_data.drive.steering_angle = self.rabacon_angle
