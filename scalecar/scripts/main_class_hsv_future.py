...
class Controller():
    ...
    def __init__(self):
        ...
        # initialization
        # competition dayif (len(_data.fiducials) > 0 ) :
            ...
            if self.pub_cnt > 10:
                ...
                self.pub_cnt = 0 = 0.3 # rabacon speed
        ...
        rospy.Subscriber("/fiducial_vertices", FiducialArray, self.child_callback)


    def child_callback(self, _data) :
        print(self.sign_id, '############################')
...
# ========== preprocessing via HSV ==========================
        ...
        # gym
        # lower_lane = np.array([213, 222, 219]) # 
    ...
    def lane_detection(self, lane_image) :
        ...
        # crop_coordinates = (300, 170, 100, 290)
    ...
    def stop_traffic(self, lane_image) :
        ...
        # # 흰색 픽셀의 개수를 세기 위한 변수 초기화
        # white_pixels = 0

        # # 이미지를 순회하며 흰색 픽셀의 개수 계산
        # for y in range(height):
        #     for x in range(width):
        #         pixel = image[y, x]
        #         if (pixel == [255, 255, 255]).all():  # 픽셀이 흰색인지 확인
        #             white_pixels += 1
    ...
    def follow_lane(self): # 차선데이터를 받아서 차선을 따라서 주행하도록 하는 함수
        ...
        if self.slow_down_flag == 1:
            ...
            if self.sign_data != 0:
                ...
                ##### 정지선 보기전에 천천히 가는 구간 #####
                if slow_flag == 1 and self.stop_flag == 0: 
                ...
                while self.stop_flag == 1 and t2-self.slow_t1 <= 15 : # 수정
                    rospy.loginfo("************* SLOW DOWN *****************")
                ...
                #### 미션 끝나는 지점에 있는 가로 흰색 실선과 Aruco Marker 인식 후 정속주행 ####
                if self.slow_flag == 2 and self.sign_data != 2 :
                    print('다시 빨라짐')
                    publishing_data.drive.speed = self.speed_lane
        ...
        # obstacle and rabacon
            ...
            elif abs(statistics.mean(self.y_list_sort[0:1]) - statistics.mean(self.y_list_sort[-2:-1])) >= 0.17 or abs(self.y_list_sort[10]) > 0.20:
                self.dynamic_flag = 1
                self.static_flag = 0
                # self.y_list.clear
        ...
        try :
            ...
            if _data.data == 1:
                ...
                if self.child_cnt >=20 :
                    ...
                    self.child_cnt = 0
            else if  _data.data == 2 and self.slow_down_flag == 1:
                self.child_cnt += 1
    ...
    def rabacon_drive(self) :
        ...
        publishing_data.drive.steering_angle = self.rabacon_mission_flag * -1.5
