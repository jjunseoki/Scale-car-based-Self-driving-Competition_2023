...
{ 
    ...
    ec.setClusterTolerance (0.08);            // set distance threshold = 1.5m 안에 있으면 같은 클러스터 분류로 하겠다.
    ec.setMinClusterSize (7);               // set Minimum Cluster Size  
    ec.setMaxClusterSize (35);            // set Maximum Cluster Size 
...
{ 
    ...
    ros::Subscriber sub = nh.subscribe<sensor_msgs::PointCloud2>("/passPC1", 1, callbackFcn);


    // << Publish Topic >> 
    // topic name : /cluster1PC 
    // topic type : sensor_msgs::PointCloud2 
    pub1 = nh.advertise<sensor_msgs::PointCloud2>("/clusterPC", 1);
